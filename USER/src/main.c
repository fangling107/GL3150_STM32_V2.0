...
// 			}
			//用1个传感器只启动一个下游的计数器
			if(RTC_12000MS&&CLJ_START==0)//启动下游计数器
			{
				RTC_12000MS=0;					
				U2DS.SedOrd = 0;	
				Sendout_uart1_CLJ260(CLJ_R260_DownAddr,0x02);	//启动下游计数器开始测量
			}
// 			if(RTC_12000MS&&CLJ_START==2&&CLJ_close==0)//每10S读取1次上下游计数器的值和流量的值
// 			{
// 				RTC_12000MS=0;			
// 				U2DS.SedOrd = 0	;	
...
// 			}
			//用1个传感器只读取一个下游的计数器
			if(RTC_12000MS&&CLJ_START==2&&CLJ_close==0)
			{
				RTC_12000MS=0;			
				U2DS.SedOrd = 0	;	
...
// 						RTC_250MS=0;								
// 						U2DS.SedOrd = 0	;	
// 						Sendout_uart1_CLJ260(CLJ_R260_UpAddr,0x03);		//关闭上游计数器
// 					}
// 					else if(RTC_250MS&&CLJ_close==1)
// 					{
// 						RTC_250MS=0;
// 						U2DS.SedOrd = 0	;
// 						Sendout_uart1_CLJ260(CLJ_R260_DownAddr,0x03);	//关闭下游计数器
// 					}
					if(RTC_250MS&&CLJ_close==0)//关闭下游计数器
					{
						RTC_250MS=0;
						U2DS.SedOrd = 0	;
...
{	
	...
	press=press_k*(pressADvalue-press_zeroAD);//计算出实际压力
...
void menu_start(void)
{
	Show_Str(240,50,lcddev.width,32,"GL3150过滤效率测试仪",32,1);		
...
void menu1(void)
{
	...
	Show_Str(77,327,800,32,"校准",32,1);
	Show_Str(77,407,800,32,"帮助",32,1);
	Show_Str(240,0,800,24,"GL3150过滤效率测试仪",24,1);
	Show_Str(492,25,lcddev.width,16,"v2.0",16,1);	
	gui_draw_hline(240,24,290,RED);
	...
	LCD_Fill(260,148,360,168,LGRAY);//小填充矩形
	...
	LCD_Fill(260,228,360,248,LGRAY);//小填充矩形
...
void No_LoadTest_orclearMenu(void)
{
	...
	while(flag)
	{
			...
			ZD_Port();					//;终端端口通讯 
			DataProcess();			//;终端数据处理及数据发送			
			if(RTC_250MS&&(Stop_flag==1||Test_over==1)&&CLJ_close==0)
			{
				RTC_250MS=0;					
				Sendout_uart1_CLJ260(CLJ_R260_DownAddr,0x03);	//停止下游计数器				
				continue;				
			}
			else if((Stop_flag==1||Test_over==1)&&RTC_250MS&&CLJ_close==2)
			{						
				RTC_250MS=0;
				CLJ_START=0;
				CLJ_close=0;
			...
			else if(RTC_12000MS&&CLJ_START==0)//延时十秒 启动下游计数器
			{	
				RTC_12000MS=0;
